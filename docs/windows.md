...
Для OCR не нужны EXE, PowerShell, Python и системный Tesseract. Установите MAX Chat Local Exporter в Chrome, соберите чат и нажмите «Запустить OCR». Расширение обработает ZIP прямо в Chrome и скачает `messages_ocr.json`.

Ранее скачанный архив можно обработать кнопкой «Выбрать ZIP для OCR». Переписку собирать повторно не требуется.

## Необязательный автономный EXE

Локальный файл `releases/packages/LeadBridgeKSO-Setup-Windows-v8.2.10.0848.exe` откройте двойным кликом и подтвердите стандартный запрос Windows на права администратора. EXE намеренно не хранится в GitHub-репозитории. Установщик работает автономно и сам:
...
## Необязательный Python OCR

Следующие команды нужны только для старого ручного процессора. Для OCR внутри расширения этот раздел можно пропустить.
